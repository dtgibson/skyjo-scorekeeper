...
- **Scoring view**: shows standings sorted ascending (lowest score first), leader highlighted with green dot and tinted row background, round number in nav bar, Undo button (disabled on round 1), "Enter Round N Scores" primary button
...
### Doubling rule: only applies when Skyjo caller is not the minimum scorer
If the player who called Skyjo has the same score as the minimum, no doubling occurs. Implemented in `GameSession.commitRound` by comparing `raw > minRaw` (strict greater-than).
...
The `.xcodeproj` package was regenerated by hand after the original was accidentally deleted. The source of truth for which files exist is the iCloud directory; the project file references them by path. The git repo uses a nested `SkyjoScorekeeper/SkyjoScorekeeper/` layout (standard Xcode convention); the iCloud working directory uses a flat layout with source files alongside the `.xcodeproj`.
